...
class BackTest(trader.Trader):
    def __init__(self, config, net_dir=None, agent=None, agent_type="nn"):
        ...
        if agent_type == "nn":
            ...
            raise ValueError()
        self.__test_set = data_matrices.get_test_set()  # backtest only needs test set
    ...
    def test_pc_vector(self):
        # return the price of test set, format: array
        return np.array(self.__test_pc_vector, dtype=np.float32)
    
    @property
    def turn_over(self): 
        return np.mean(self.__turn_over)
    ...
    def generate_history_matrix(self):
        # construct the history data matrix
    ...
    def trade_by_strategy(self, omega):
        ...
        future_price = np.concatenate((np.ones(1), self.__get_matrix_y()))  # np.ones(1) as BCT, which is a baseline asset
        ...
        portfolio_change = pv_after_commission * np.dot(omega, future_price) # portfolio change ratio
