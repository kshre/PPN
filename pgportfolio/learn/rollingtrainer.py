...
class RollingTrainer(TraderTrainer):
    ...
    def __rolling_logging(self):
        # log the information of training, valiadation, test
    ...
    def rolling_train(self, online_w=None): 
        ...
        if steps > 0:
            self._matrix.append_experience(online_w)      # enrich the experience 
            for i in range(steps):
                x, y, last_w, w = self.next_batch()
                self._agent.train(x, y, last_w, w)     # train the agent
