...
def train_one(save_path, config, log_file_dir, index, logfile_level, console_level, device):
    ...
    if log_file_dir:
        ...
        console = logging.StreamHandler()   # output the logging information to sys.stdout, sys.stderr  
...
def train_all(processes=1, device="cpu"):
    """
    aim: train all the agents in the train_package folders
    ...
    for dir in all_subdir:
        ...
        if not (os.path.isdir("./"+train_dir+"/"+dir+"/tensorboard") or os.path.isdir("./"+train_dir+"/"+dir+"/logfile")):
            # allocate process function and initialize 
            ...
            pool.append(p) # append the process into the pool
        else:
            continue
        ...
        wait = True   
        while wait:       
            time.sleep(5)      # waiting scheme
            for p in pool:       
                alive = p.is_alive()
                if not alive:        # if the process is not alive, then remove
                    pool.remove(p)
