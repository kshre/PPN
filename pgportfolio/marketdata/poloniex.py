...
class Poloniex:
    def __init__(self, APIKey='', Secret=''):
        ...
        # Timestamp becomes String
        self.str_timestamp = lambda datestr=self.timestamp_str(), format="%Y-%m-%d %H:%M:%S": int(time.mktime(time.strptime(datestr, format)))
        # String becomes Timestamp
        self.float_roundPercent = lambda floatN, decimalP=2: str(round(float(floatN) * 100, decimalP))+"%"
        # discrete values become percentages

        # PUBLIC COMMANDS
        self.marketTicker = lambda x=0: self.api('returnTicker')  # Ticker 
        self.marketVolume = lambda x=0: self.api('return24hVolume')  # 24h volumn
